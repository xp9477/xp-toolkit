...
class Script:
    ...
    def run(self) -> bool:
        ...
        try:
            r1 = self.session.get(f"{self.base_url}/member.php?mod=logging&action=login", timeout=15)
            r1.encoding = "utf-8"
        except Exception as e:
            print(f"访问登录页面失败: {e}")
            return False

        formhash_match = re.search(r'formhash=([a-zA-Z0-9]+)', r1.text) or re.search(r'name="formhash"\s+value="([a-zA-Z0-9]+)"', r1.text)
        ...
        # 验证是否登录成功
        if "欢迎您回来" not in r2.text and "登录成功" not in r2.text and "现在将转入" not in r2.text:
            cdata_match = re.search(r'<!\[CDATA\[(.*?)\]\]>', r2.text, re.DOTALL)
            msg_raw = cdata_match.group(1) if cdata_match else "未知原因"
            msg_no_html = re.sub(r'<script.*?>.*?</script>', '', msg_raw, flags=re.DOTALL)
            msg_no_html = re.sub(r'<.*?>', '', msg_no_html)
            login_lines = [line.strip() for line in msg_no_html.splitlines() if line.strip() and not line.strip().isdigit()]
        ...
        try:
            r3 = self.session.get(f"{self.base_url}/plugin.php?id=k_misign:sign", timeout=15)
            r3.encoding = "utf-8"
        except Exception as e:
            print(f"访问签到页面失败: {e}")
            return False

        if "您的签到排名" in r3.text or "今日已签" in r3.text or "已累计签到" in r3.text:
            print("今天已经签到过了")
            return True

        sign_formhash_match = re.search(r'formhash=([a-zA-Z0-9]+)', r3.text) or re.search(r'name="formhash"\s+value="([a-zA-Z0-9]+)"', r3.text)
        ...
        sign_msg_no_html = re.sub(r'<script.*?>.*?</script>', '', sign_msg_raw, flags=re.DOTALL)
